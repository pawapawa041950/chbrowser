...
**境界の確定化**: 旧コードは `agentFinalBoundary` を char 単位で投機セット → 巻き戻ししていた。新エンジンは **最初の `StreamBody` で work↔body 境界を確定**する。
ただしストリーミングでは「そのラウンドが tool 呼び出しか最終回答か」はラウンド完了まで分からないため、各ラウンドは作業エリアへ流し、最終回答 / `ask_user` と判明した時点で `IAgentHost.WorkCheckpoint()` / `RollbackWork()` で作業エリア分を巻き戻して本文へ移す (B4 実装で確定。旧来の char-index 直接操作よりは軽いが、rollback 自体は必要)。Strategist の `<think>` は最終回答エリアからは除去される (MarkdownRenderer)。

**エラー提示 (D15)**

原則: **`Error()` (赤・ターン中断) は「エージェントが推論で扱えない基盤障害」だけ。エージェントが対処できる失敗 (tool エラー・タスク失敗) は作業ログに残し、Strategist が本文で要約する** (= 最下層で扱える所で扱う層思想の UX 版)。

> **スレッド注意 (B4/B5 実装)**: host (= AiChatViewModel) は WPF UI スレッドでしか触れない (`PostToShell` が `CoreWebView2` に直接アクセス)。新エンジンは `SendAsync` (UI スレッド) 発で全 `await` を `ConfigureAwait(true)` にし継続を UI スレッドに留める (既存ループと同じ)。
> **B5 の並列実行は「UI スレッド上の async 並行」方式を採用**した: 1 ターンの複数 dispatch_task を `Task.WhenAll` で同時 await すると、各 `ChatStreamAsync` の HTTP が同時に in-flight になる (= 複数 LLM リクエストが並列に走る) が、`ChatStreamAsync` の読み取りループは `ConfigureAwait(true)` なので継続も onDelta も UI スレッドに戻る。**バックグラウンドスレッドを使わないので Dispatcher marshaling も archive ロックも不要**。視覚的な区画分離 (並行ストリームが混ざらない) のため host は区画ごとにバッファを分ける。
...
interface IAgentHost
{
    ...
    int  WorkCheckpoint();                            // rollback 用 (最終回答判明時に作業→本文へ移す)
...
- ✅ **B1 完了 (2026-05-23)** — `AppConfig` に新エンジン用設定を additive 追加: `UseNewAgentEngine` (切替トグル) / Strategist 接続 4 項目 / Worker 接続 4 項目 / `AllowParallelWorkers`。`SettingsViewModel` (ObservableProperty + ctor init + ToConfig) と `AiPanel.xaml`(.cs) に配線 (API キーは 3 つとも PasswordBox 同期)。現状は保存のみ・エンジン切替は未配線 (B6 で有効化)。ビルド 0 警告 / 0 エラー。
- ✅ **B2 完了 (2026-05-23)** — `Services/Agent/ToolRuntime.cs` 新設。`AgentToolContext` の `ThreadToolset` / `ChatArchive` を包み、`ExecuteAsync(name, args, taskId, ct) → ToolOutput` で検証・実行・正規化 (error JSON 判定)・archive 記録 (id 返却) を行う純粋コード層。`GetWorkerToolDefinitions()` はスレ系 + `recall_archive` (id 指定) を返し `list_archive` は除外 (隔離・レビュー①)。estimate ゲートなし。archive 記録は逐次前提 (並列の thread-safe 化は B5)。ビルド 0 警告 / 0 エラー。
- ✅ **B3 完了 (2026-05-23)** — `Services/Agent/Worker.cs` 新設。使い捨て ReAct ループ。`RunAsync(TaskSpec, IWorkSection, ct) → TaskResult`: 新鮮な会話 (system + タスク) で Worker 接続 LLM を回し、tool_call を `ToolRuntime.ExecuteAsync` で実行、`submit_result` で終端。停止条件: submit_result (done/partial/failed) / 予算超過 → partial / ラウンド上限 → partial / LLM 失敗 → failed。ツールなしテキストは done 扱いの寛容フォールバック (D10)。作業ログは `IWorkSection` に stream / ToolMarker / Complete。evidenceIds は実行で集めた archiveId 群。ビルド 0 警告 / 0 エラー。
- ✅ **B4 完了 (2026-05-24) = 初の実行可能点** — `Strategist.cs` (L3 tool-loop: create_plan / revise_plan / dispatch_task / ask_user / recall_archive + テキスト=最終回答、heavy ゲート=累積予算、D9 再委譲上限、D13 リセット、永続会話 D8) + `NewAgentEngine.cs` (Strategist 保持 + dispatch ごとに使い捨て Worker 起動・ToolRuntime 共有) + `AiChatViewModel.AgentHost.cs` (partial で `IAgentHost` 実装・既存イベント + `MarkdownRenderer` 再利用・work/body 2 バッファ) + `LlmSettings.StrategistFromConfig/WorkerFromConfig` (未設定は Llm* にフォールバック) + VM ctor に `AppConfig` 追加し `UseNewAgentEngine` で `_engine` 切替 + `MainViewModel.Threads` で `CurrentConfig` 渡し。`UserMessageAdded` は両エンジン共通化のため `SendAsync` へ移動。ビルド 0 警告 / 0 エラー。設定で ON にし AI チャットを開き直すと新エンジンで動く。**簡易点 (B4): 並列は逐次のみ (D7→B5)、plan は新エンジン独自の簡易モデル (PlanToolset 非使用)、区画は逐次追記、最終回答は round 確定後に本文へ移すため token ストリームは作業エリア側で見える。**
- ✅ **B5 完了 (2026-05-24)** — 並列実行。当初計画 (バックグラウンドスレッド + Dispatcher marshaling + archive ロック) から **「UI スレッド上の async 並行」方式に変更** (= より安全・単純)。`AllowParallelWorkers` ON かつ 1 ターンに dispatch_task が複数あると、Strategist が `Task.WhenAll` で同時 await し各 Worker の HTTP を同時 in-flight にする (継続/onDelta は UI スレッドに留まるためマーシャリング/ロック不要)。host は区画ごとにバッファ (`SectionBuf`) を分け、並行ストリームが混ざらないようにした。gate/予算/区画作成は `PrepareDispatch` で逐次確定 → Worker 実行のみ並行。Strategist プロンプトに「独立タスクは複数 dispatch_task を同時に呼ぶと並列」明記。tool 予算も 2 倍化済 (既定12/heavy24)。ビルド 0 警告 / 0 エラー。
- ✅ **B6 完了 (2026-05-24) = 昇格・旧撤去** — 新エンジンを本機能に昇格し旧エンジンを削除。`ExistingAgentEngine.cs` 削除、`AiChatViewModel` を新エンジン専用に全面書き換え (旧単一ループ・review/estimate ゲート・承諾分類器・trim ハック・`_history`/`_pendingReviewPrompts` 等を全削除)、`UseNewAgentEngine` トグルと `MainViewModel.BuildAiSystemPrompt` (旧 system プロンプト) を削除。`SwitchContext` は `AgentToolContext.Thread` を差し替えて新エンジンに追従させる (`ToolRuntime` は `ctx.Thread` を動的参照・会話は保持)。AI チャットは常に 3 レイヤーエージェントで動作。ビルド 0 警告 / 0 エラー。**→ 構築計画 B0〜B6 すべて完了。**

> 補足: ユーザ評価では新エンジンは旧実装より精度がやや低いが消費トークンは数分の 1。精度↔コストの最適化は今後の課題 (プロンプト改善・モデル選択・finding の質など) として継続。

---

*最終更新: 2026-05-24 / 設計確定 (D1〜D15) + 実装 B0〜B6 すべて完了 (B6 = 新エンジン昇格・旧エンジン撤去)。AI チャットは 3 レイヤーエージェント単独で稼働。今後は精度↔コスト最適化が課題*
